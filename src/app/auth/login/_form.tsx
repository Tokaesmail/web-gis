...
function Field({ label, name, type, placeholder, autoComplete }: {
  ...
  return (
    <div className="space-y-1.5">
      <label htmlFor={name} className="block text-[0.72rem] text-slate-400 tracking-wide uppercase">{label}</label>
      <input id={name} name={name} type={type} placeholder={placeholder} autoComplete={autoComplete} required
