...
// ─── Schema (inline so no import path issues) ──────────────────────────────────
      ...
      <div className="space-y-2">
        ...
        {password.length > 0 && (
          <div className="space-y-1.5 px-0.5">
            ...
            <p className={`text-[0.65rem] ${strength.color}`}>{strength.label}</p>
          </div>
        )}
      </div>

      <SubmitButton pending={isSubmitting} label="Create Account" pendingLabel="Creating account..." />
    </form>
  );
}
...
function getPasswordStrength(password: string) {
  ...
  return {
    score,
    label: ["", "Weak", "Fair", "Good", "Strong"][score] || "Weak",
    color: ["", "text-red-400", "text-orange-400", "text-yellow-400", "text-emerald-400"][score] || "text-red-400",
  };
}
...
const Field = ({ label, id, error, ...props }: FieldProps) => (
  <div className="space-y-1.5">
    <label htmlFor={id} className="block text-[0.72rem] text-slate-400 tracking-wide uppercase">{label}</label>
    <input id={id} {...props}
...
interface PasswordFieldProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label: string; id: string; show: boolean; onToggle: () => void; error?: string;
}
const PasswordField = ({ label, id, show, onToggle, error, ...props }: PasswordFieldProps) => (
  <div className="space-y-1.5">
    <label htmlFor={id} className="block text-[0.72rem] text-slate-400 tracking-wide uppercase">{label}</label>
    <div className={`flex items-center gap-2 bg-white/3 border rounded-xl px-3.5 py-3 transition-all ${
      error
