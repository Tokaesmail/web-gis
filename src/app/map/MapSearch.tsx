...
export default function MapSearch({
  ...
  if (/^-?\d+(\.\d+)?\s*,\s*-?\d+(\.\d+)?$/.test(value)) {
    handleCoordinateSearch();
      ...
      {open && (
        <div className="mt-1.5 bg-[#0a1628] border border-white/10 rounded-xl shadow-[0_16px_48px_rgba(0,0,0,0.6)] overflow-hidden max-h-[280px] overflow-y-auto app-scroll">          {results.map((r, i) => (
            <button
              key={i}
