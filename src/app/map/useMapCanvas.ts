...
export function useMapCanvas() {
  ...
  const drawCircle = useCallback((
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.save();
    ctx.beginPath(); ctx.arc(center.x, center.y, radiusPx, 0, Math.PI * 2); ctx.clip();
  ...
  const drawMarker = useCallback((canvas: HTMLCanvasElement, p: { x: number; y: number }) => {
    const ctx = canvas.getContext("2d");
    if (!ctx) return;
    // مش بنمسح عشان الـ markers بتتراكم
  ...
  // ── Capture: crop الـ overlay على الـ bounding box وحفظه في IndexedDB ───
  const capture = useCallback(async (
    ...
    const w    = Math.max(1, Math.min(overlayCanvas.width,  Math.ceil(Math.max(...xs))) - minX);
    const h    = Math.max(1, Math.min(overlayCanvas.height, Math.ceil(Math.max(...ys))) - minY);
    ...
    cCtx.drawImage(overlayCanvas, -minX, -minY);
  ...
  // ── captureCircle: يحوّل الـ circle لـ polygon points عشان الـ crop ──────
  ...
  //   image       → PNG blob
  //   coordinates → [{lat, lng}, ...]   ← خطوط الطول والعرض
  //   metadata    → {areaName, areaSizeHa, zoom, capturedAt, toolType}
