      ...
      <div className="flex items-center gap-0.5 bg-[#0a1628]/95 backdrop-blur-md border border-white/10 rounded-full px-1 py-1 shadow-lg">
        {SATELLITES.map((s) => (
          <button key={s} onClick={() => setSat(s)}
            className={`text-[0.67rem] px-3 py-1 rounded-full cursor-pointer transition-all ${sat === s ? "bg-white/10 text-slate-100" : "text-slate-500 hover:text-slate-300"}`}>
            {s}
          </button>
        ))}
      </div>
      <div className="flex items-center gap-0.5 bg-[#0a1628]/95 backdrop-blur-md border border-white/10 rounded-full px-1 py-1 shadow-lg">
        {INDICES.map((i) => (
          <button key={i} onClick={() => setIdx(i)}
            className={`text-[0.67rem] px-3 py-1 rounded-full cursor-pointer transition-all ${idx === i ? "bg-cyan-400/20 text-cyan-400 border border-cyan-400/30" : "text-slate-500 hover:text-slate-300"}`}>
            {i}
          </button>
        ))}
      </div>
...
function AITriggerButton({ onClick, active }: { onClick: () => void; active: boolean }) {
  return (
    <button onClick={onClick} title="AI Assistant"
      className={`absolute bottom-5 right-[60px] z-[1000] w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer transition-all shadow-lg pointer-events-auto ${active ? "bg-cyan-400 text-[#040d1a] shadow-[0_0_20px_rgba(0,212,255,0.5)]" : "bg-[#0a1628]/95 backdrop-blur-md border border-white/10 text-slate-400 hover:text-cyan-400 hover:border-cyan-400/30"}`}>
        ...
        {tools.map((tool) => (
          <div key={tool.id} className="relative group">
            <button onClick={() => onToolChange(tool.id)}
              className={`w-9 h-9 rounded-lg flex items-center justify-center transition-all cursor-pointer ${activeTool === tool.id ? "bg-cyan-400 text-[#040d1a] shadow-[0_0_12px_rgba(0,212,255,0.5)]" : "text-slate-400 hover:text-slate-100 hover:bg-white/[0.08]"}`}>
              {tool.icon}
            </button>
...
// ─── Search (Nominatim — real geocoding, any place on Earth) ─────────────────
...
// ─── Leaflet Map with real drawing + search ───────────────────────────────────
function LeafletMap({ activeTool, onAreaSelected, onCoordsUpdate, flyToRef, clearRef }: {
  activeTool: DrawTool;
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", { attribution: "Tiles © Esri", maxZoom: 20 }).addTo(map);
      L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}", { attribution: "", maxZoom: 20, opacity: 0.7 }).addTo(map);

      // Sample field polygon
      const fieldCoords: [number, number][] = [[30.54, 31.18], [30.56, 31.22], [30.53, 31.26], [30.50, 31.24], [30.49, 31.20], [30.52, 31.17]];
      const ndviPolygon = L.polygon(fieldCoords, { color: "#00d4ff", weight: 2, dashArray: "6 4", fillColor: "#22c55e", fillOpacity: 0.22 }).addTo(map);
      ...
      // ── Click: show coords + draw ──
      map.on("click", (e: any) => {
        const tool = activeToolRef.current;
        const { lat, lng } = e.latlng;
        onCoordsUpdate(lat, lng);
        if (tool === "pointer") return; // don't draw, but coords still shown
...
export default function MapPage() {
  ...
  // ESC → reset to pointer + clear temp drawing
  useEffect(() => {
  ...
  return (
    <div className={`flex flex-col w-full h-screen bg-[#040d1a] overflow-hidden ${isRTL ? "font-arabic" : ""}`}>
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        {/* COORDINATES POPUP — shown on every click */}
        {coords && (
        ...
        {/* ANALYSIS DASHBOARD — slides up after drawing/selecting */}
        <AnalysisDashboard
